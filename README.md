...
| Версия | N | seed | overall | Категории | Статус |
|---|---|---|---|---|---|
| v0.4.5 | 12 (3/cat) | 42 | **0.75** | KU 1.00, MS 0.67, SS 0.67, T 0.67 | замерено |
| v0.5 (без ROI) | 40 (10/cat) | 42 | **0.61** | KU 0.80, MS 0.55, SS 0.50, T 0.56 (с 62× 402-ошибками к концу) | замерено |
| **v0.5+ (с ROI)** | 12 (3/cat) | 42 | **0.71** | KU 0.83, MS **0.33**, SS 0.67, T **1.00** | **в submit** |
| v0.6 (dual-track) | 12 (3/cat) | 42 | 0.58 | KU 0.83, MS 0.33, SS **0.33**, T 0.83 | **отброшено** |
...
- multi_session **-34pp** — побочный эффект prefix-а: каждый turn
  встаёт в эмбеддинг-пространстве ближе к своей сессии, что мешает
  кросс-сессионному сбору
- overall -4pp в пределах CI на N=12 (±18pp). Real signal — это
  спред по категориям, не overall.

**Что пробовали в v0.6 (negative result, не в submit):**

Гипотеза была: убрать prefix из dense embedding, оставить только в
BM25/sparse — это должно вернуть multi_session и сохранить temporal.
По логике: dense даёт кросс-сессионную семантику (drift от prefix
бьёт по ней), а BM25 ловит именованные сущности и даты, для которых
prefix полезен.

**Гипотеза не подтвердилась:**
- multi_session **не восстановился** (0.33 → 0.33). Значит, регрессия
  multi_session не из-за prefix-drift, причина глубже — скорее всего
  в reconciliation или способе хранения cross-session фактов. Тема
  для v0.7.
- single_session **сломался** (0.67 → 0.33). Внутри одной сессии
  prefix-обогащённый dense помогает disambiguate турны, и убрать
  его означает потерять этот сигнал.
- temporal слегка просел (1.00 → 0.83) — prefix всё-таки тянет dense
  в правильное направление для дат.
- overall: **0.58**, на 13pp хуже v0.5+.

**Вывод**: prefix должен оставаться в dense. Для multi_session нужен
другой подход (например, хранить факт глобально на user_id с
session_scope=null, чтобы он не зависел от ведущей сессии). Откатили,
v0.5+ в submit.
